...
export const useTimerStore = defineStore("timer", {
  state: () => {
    ...
    const DEFAULT_PRESETS = [
      {
        ...
        label: "5 min",
        seconds: 300,
        icon: "@/src/assets/timers/Countdown.png",
      },
      {
        id: "p15",
        label: "15 min",
    ...
    try {
      storedPresets = JSON.parse(localStorage.getItem("timer-presets"));
    } catch {}

    return {
      presets: storedPresets?.length ? storedPresets : DEFAULT_PRESETS,
  ...
  actions: {
    ...
    applyPresetToWidget(widgetId, seconds, icon = null) {
      const widget = this.widgets.find((w) => w.id === widgetId);
    ...
    addPreset({ label, seconds, icon = null }) {
    ...
    updatePreset(presetId, updates) {
      ...
      if (updates.label !== undefined) preset.label = updates.label;
      if (updates.seconds !== undefined) preset.seconds = updates.seconds;
